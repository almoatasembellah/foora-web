...
class ScouterController extends Controller
{
    ...
    public function scoutePlayer(RatePlayerRequest $request)
    {
        ...
        if (!$isPlayerRated) {
            ...
            $user->update(['overall' => PlayerRateService::getPlayerRateOverall($user->rates)['overall']]);
    ...
    public function getScoutedMatches(Request $request)
    {
        $gamesAttended = GameAttendance::with('game')->where('scouter_id', $request->user()->id)->get();

        $games = [];

        foreach ($gamesAttended as $game){
            $games[] = GameResource::make($game['game']);
        }

        return $this->sendResponse($games, 'Scouted players fetched successfully.');
